...
class GraphsControllerTest < ActionController::TestCase
  ...
  test "should create graph" do
    assert_difference('Graph.count') do
      post :create, graph: { dataset_id: @graph.dataset_id, graph_type_id: @graph.graph_type_id, name: @graph.name }
    end
  ...
  test "should update graph" do
    patch :update, id: @graph, graph: { dataset_id: @graph.dataset_id, graph_type_id: @graph.graph_type_id, name: @graph.name }
    assert_redirected_to graph_path(assigns(:graph))
  end
